...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        for (int i = 0; i < humanRelationships.length; i++) {
            if (i < index && humanRelationships[index][i]) set.add(i);
            if (i > index && humanRelationships[i][index]) set.add(i);
        }
        // deep > 1
        for (int i = 1; i < deep; i++) {
            for (Object o : set.toArray()) {
                ...
                for (int j = 0; j < humanRelationships.length; j++) {
                    if (j < v && humanRelationships[v][j]) set.add(j);
                    if (j > v && humanRelationships[j][v]) set.add(j);
                }
            }
        }
        set.remove(index);
